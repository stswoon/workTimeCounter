...
import {
  getWorkTimeCounterData,
  saveWorkTimeCounterData,
...
const TimerManager: FC = () => {
  ...
  const setTimerName = useCallback(
  ...
  const removeTimer = useCallback(
    (id: string) => {
      const newTimerModels = timerModels.filter(
        (timerModel) => timerModel.id !== id,
      );
      setTimerModels(newTimerModels);
      saveToLocalStorage(newTimerModels);
    },
  ...
  const addNewTimer = useCallback(() => {
    const newTimerModels = [
      ...timerModels,
      createTimer(timerModels.length + 1),
    ];
    setTimerModels(newTimerModels);
    saveToLocalStorage(newTimerModels);
  }, [saveToLocalStorage, timerModels]);
    ...
    >
      ...
      {displayAddButton && (
        <Box sx={{ display: "flex", justifyContent: "center" }}>
          <Button variant="outlined" onClick={() => addNewTimer()}>
            {strings.addTimer}
          </Button>
        </Box>
      )}
